...
#define CONFIG_XML_NAME "configure.xml"
...
Configure::Configure():m_homeDir(""),m_configFile(""),m_srcLan(""),m_detLan("")
,m_dirty(false)
{
}
...
void Configure::initialization()
{
    ...
    m_configFile = m_homeDir + "/" + CONFIG_XML_NAME;
    g_log(LOG_INFO, "home direcotry:(%s)\n", m_homeDir.c_str());
    if (!Util::isDir(m_homeDir)) {        
        Util::copyDir("etc", m_homeDir);
        g_log(LOG_INFO, "configure file:(%s)\n", m_configFile.c_str());
    } else if (!Util::isFile(m_configFile)) {
        string path = "etc/";
        path += CONFIG_XML_NAME;
        Util::copyFile(path, m_configFile);
    }
...
{
    string dictDir = m_homeDir + "/" + DICT_DIR_NAME;
    scanDictDir(dictDir);
}
...
void Configure::loadLanguage(string& path)
{
